...
class Storage:
    ...
    def get(self, key: str):
        """Получить значение(я) по ключу (префиксу ключа)"""
        if key not in self:
            current_keys = [i for i in self.data.keys() if key in i]
            pairs = [f"{key}: {self.data[key]}" for key in current_keys]
            if len(pairs):
                return '\n'.join(pairs)
    ...
    def get_key(self, value: str):
        """Получить ключ(и) по значению"""
        keys = [i for i in self.data.keys() if self.data[i] == value]
        if len(keys) == 0:
            LOGGER.warning(f'There is no key with the '
                           f'value "{value}" in data file "{self.name}". Skip')
            return str(exceptions.NoSuchValueError(self.name, value))
        LOGGER.info(f'Get key(s) {keys} from the value {value}')
        return '\n'.join(keys)
